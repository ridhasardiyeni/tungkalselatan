...
if ($_GET['aksi'] == 'input') {
    ...
    $path = "images_berita/" . $fotobaru;
    if ($judul != null and $keterangan != null and $foto != null) {
        if (move_uploaded_file($tmp, $path)) {
            $sql_input = "INSERT INTO `berita` (`id`, `judul`, keterangan,`foto`) VALUES "
                    . "(NULL, '$judul', '$keterangan', '$fotobaru')";
    ...
    if ($foto != null) {
        ...
        if ($judul != null and $keterangan != null) {
            if (move_uploaded_file($tmp, $path)) {
                $sql_foto = "SELECT * from berita WHERE id='$id'";
                $query_foto = mysql_query($sql_foto);
                $ambil_foto = mysql_fetch_array($query_foto);

                if (is_file("images_berita/" . $ambil_foto['foto'])) {
                    unlink("images_berita/" . $ambil_foto['foto']);
                }
                $sql_edit = "UPDATE berita SET judul = '$judul',keterangan='$keterangan',foto='$fotobaru' WHERE id = '$id' ";
                ...
                echo "<script>alert('Foto Gagal di upload Ukuran Terlalu Besar!');history.go(-1);</script>";
            }
        } else {
            echo "<script>alert('Tidak Boleh Ada Yang Kosong!');history.go(-1);</script>";
        }
        ...
        if ($judul != null  and $keterangan != null) {
            $sql_edit = "UPDATE berita SET judul = '$judul',keterangan='$keterangan' WHERE id = '$id' ";
                $edit = mysql_query($sql_edit);
            if ($edit) {
                header("location:index.php?page=berita");
            } else {
