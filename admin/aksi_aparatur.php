...
if ($_GET['aksi'] == 'input') {
    ...
    $path = "images_aparatur/" . $fotobaru;
    if ($judul != null and $keterangan != null and $foto != null) {
        if (move_uploaded_file($tmp, $path)) {
            $sql_input = "INSERT INTO `aparatur` (`id`, `title`, `foto`, `keterangan`) VALUES "
                    . "(NULL, '$judul', '$fotobaru', '$keterangan')";
            $simpan_input = mysql_query($sql_input);
            if ($simpan_input) {
                header("location:index.php?page=aparatur");
            } else {
    ...
    if ($foto != null) {
        ...
        if ($judul != null and $keterangan != null) {
            if (move_uploaded_file($tmp, $path)) {
                $sql_foto = "SELECT * from aparatur WHERE id='$id'";
                $query_foto = mysql_query($sql_foto);
                $ambil_foto = mysql_fetch_array($query_foto);

                if (is_file("images_aparatur/" . $ambil_foto['foto'])) {
                    unlink("images_aparatur/" . $ambil_foto['foto']);
                }
                $sql_edit = "UPDATE aparatur SET title = '$judul',keterangan='$keterangan',foto='$fotobaru' WHERE id = '$id' ";
        ...
        if ($judul != null and $keterangan != null) {
            $sql_edit = "UPDATE aparatur SET title = '$judul',keterangan='$keterangan' WHERE id = '$id' ";
            $edit = mysql_query($sql_edit);
            if ($edit) {
                header("location:index.php?page=aparatur");
            } else {
    ...
    $sql_hapus = "DELETE FROM aparatur WHERE id = '$_GET[id]'";
