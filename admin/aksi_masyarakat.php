...
if ($_GET['aksi'] == 'input') {
    ...
    $path = "images_masyarakat/" . $fotobaru;
    if ($judul != null and $keterangan != null and $foto != null) {
        if (move_uploaded_file($tmp, $path)) {
            $sql_input = "INSERT INTO `masyarakat` (`id`, `title`, `foto`, `keterangan`) VALUES "
    ...
    if ($foto != null) {
        ...
        if ($judul != null and $keterangan != null) {
            if (move_uploaded_file($tmp, $path)) {
                $sql_foto = "SELECT * from masyarakat WHERE id='$id'";
                $query_foto = mysql_query($sql_foto);
                $ambil_foto = mysql_fetch_array($query_foto);

                if (is_file("images_masyarakat" . $ambil_foto['foto'])) {
                    unlink("images_masyarakat/" . $ambil_foto['foto']);
                }
                $sql_edit = "UPDATE masyarakat SET title = '$judul',keterangan='$keterangan',foto='$fotobaru' WHERE id = '$id' ";
        ...
        if ($judul != null and $keterangan != null) {
            $sql_edit = "UPDATE masyarakat SET title = '$judul',keterangan='$keterangan' WHERE id = '$id' ";
            $edit = mysql_query($sql_edit);
            if ($edit) {
                header("location:index.php?page=masyarakat");
            } else {
    ...
    $sql_hapus = "DELETE FROM masyarakat WHERE id = '$_GET[id]'";
