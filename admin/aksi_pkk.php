...
if ($_GET['aksi'] == 'input') {
    ...
    $path = "images_pkk/" . $fotobaru;
    if ($judul != null and $keterangan != null and $foto != null) {
        if (move_uploaded_file($tmp, $path)) {
            $sql_input = "INSERT INTO `pkk` (`id`, `title`, `foto`, `keterangan`) VALUES "
    ...
    if ($foto != null) {
        ...
        if ($judul != null and $keterangan != null) {
            if (move_uploaded_file($tmp, $path)) {
                $sql_foto = "SELECT * from pkk WHERE id='$id'";
                $query_foto = mysql_query($sql_foto);
                $ambil_foto = mysql_fetch_array($query_foto);

                if (is_file("images_pkk" . $ambil_foto['foto'])) {
                    unlink("images_pkk/" . $ambil_foto['foto']);
                }
                $sql_edit = "UPDATE pkk SET title = '$judul',keterangan='$keterangan',foto='$fotobaru' WHERE id = '$id' ";
        ...
        if ($judul != null and $keterangan != null) {
            $sql_edit = "UPDATE pkk SET title = '$judul',keterangan='$keterangan' WHERE id = '$id' ";
            $edit = mysql_query($sql_edit);
            if ($edit) {
                header("location:index.php?page=pkk");
            } else {
    ...
    $sql_hapus = "DELETE FROM pkk WHERE id = '$_GET[id]'";
